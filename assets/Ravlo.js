...
let currentInvitees       = [];
let recurrence            = { type: 'none', interval: 1, days: [], smartInterval: 'weekly' };
...
var gregState = { gy: 2026, gm: 0, selectedGd: null };
...
/* =========================== HOLIDAYS ============================ */
const GREG_HOLIDAYS = {
    '01-01': "New Year's Day", '02-14': "Valentine's Day", '03-08': "Int'l Women's Day",
    '03-20': "Int'l Day of Happiness", '03-21': "Nowruz (Spring Equinox)", '04-01': "April Fools' Day",
    '04-22': "Earth Day", '05-01': "International Workers' Day", '06-05': "World Environment Day",
    '06-21': "World Music Day", '07-04': "US Independence Day", '08-12': "Int'l Youth Day",
    '09-21': "Int'l Day of Peace", '10-31': "Halloween", '11-11': "Veterans Day",
...
async function fetchEvents() {
    ...
    const { data, error } = await sb.from('ravlo').select('*').eq('user_id', currentUser.id).neq('invitation_status', 'declined').order('start_date', { ascending: true });
...
function updateRecurrenceModalUI() {
    ...
    if (showInterval && intervalLabel) {
        const map = { daily: 'day(s)', weekly: 'week(s)', monthly: 'month(s)', yearly: 'year(s)', custom: 'week(s)' };
...
function renderYearPanel(container, selectedYear, fromYear, toYear, persian, onSelect) {
    ...
    for (var y = fromYear; y <= toYear; y++) {
        ...
        (function(year){ btn.addEventListener('click', function(e){ e.stopPropagation(); onSelect(year); }); })(y);
        container.appendChild(btn);
    }
    setTimeout(() => { var s = container.querySelector('.selected'); if (s) s.scrollIntoView({ block: 'center' }); }, 0);
}
...
function renderGregPicker() {
    var gy = gregState.gy, gm = gregState.gm;
    ...
    var today = new Date(), daysInMonth = new Date(gy, gm + 1, 0).getDate(), firstDay = new Date(gy, gm, 1).getDay();
    gregDaysEl.innerHTML = '';
    var prevDays = new Date(gy, gm, 0).getDate();
    for (var i = 0; i < firstDay; i++) { var el = document.createElement('div'); el.className = 'picker-day other'; el.textContent = prevDays - firstDay + 1 + i; gregDaysEl.appendChild(el); }
    for (var d = 1; d <= daysInMonth; d++) {
        var el = document.createElement('div'); el.className = 'picker-day';
        el.textContent = d;
...
function openGregMonthPopup() {
    if (gregMonthPopup.classList.contains('open')) { gregMonthPopup.classList.remove('open'); return; }
    gregYearPopup.classList.remove('open');
    ...
    for (var i = 0; i < 12; i++) {
        ...
        (function(idx){ item.addEventListener('click', function(e){ e.stopPropagation(); gregState.gm = idx; renderGregPicker(); }); })(i);
...
function openGregYearPopup() {
    if (gregYearPopup.classList.contains('open')) { gregYearPopup.classList.remove('open'); return; }
    gregMonthPopup.classList.remove('open');
    gregYearPopup.classList.add('open');
    renderYearPanel(gregYearPanel, gregState.gy, 2000, 2040, false, function(y){ gregState.gy = y; renderGregPicker(); });
}
...
function renderGregorianMonth() {
    var year = currentDate.getFullYear(), month = currentDate.getMonth(), today = new Date();
    ...
    ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].forEach(d => {
        var el = document.createElement('div'); el.className = 'day-name'; el.textContent = d; calendarGrid.appendChild(el);
    });
    ...
    var total = firstDay + daysInMonth, trailing = (7 - (total % 7)) % 7;
...
function makeGregCell(year, month, day, otherMonth, isToday) {
    var normDate = new Date(year, month, day), ny = normDate.getFullYear(), nm = normDate.getMonth(), nd = normDate.getDate();
    var cell = document.createElement('div');
    cell.className = 'day-cell' + (otherMonth ? ' other-month' : '') + (isToday ? ' today' : '');
    var numEl = document.createElement('span'); numEl.className = 'day-number'; numEl.textContent = day; cell.appendChild(numEl);
    var holiday = getGregHoliday(ny, nm, nd);
    if (holiday) { var hl = document.createElement('span'); hl.className = 'holiday-label'; hl.textContent = holiday; cell.appendChild(hl); }
...
dayEvents.forEach(ev => {
    ...
    cell.addEventListener('click', () => {
        ...
        viewMode = 'day'; syncViewTabsUI(); renderCalendar();
    });
    return cell;
}
...
evEl.addEventListener('click', function(e) {
    ...
    if (item.ev.invitation_status === 'pending') {
        ...
        currentMonthYearEl.textContent = viewDate.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    });
}
/* ------------------------- YEAR VIEW ------------------------- */
...
function makeYearMonthCard_Greg(year, month, today) {
    var card = document.createElement('div'); card.className = 'year-month-card';
    var nameEl = document.createElement('div'); nameEl.className = 'ym-name';
    nameEl.textContent = new Date(year, month).toLocaleString('en-US', { month: 'short' }); card.appendChild(nameEl);
    var daysInMonth = new Date(year, month + 1, 0).getDate(), firstDay = new Date(year, month, 1).getDay();
    var miniGrid = document.createElement('div'); miniGrid.className = 'ym-grid';
    for (var i = 0; i < firstDay; i++) miniGrid.appendChild(document.createElement('span'));
    for (var d = 1; d <= daysInMonth; d++) {
        var span = document.createElement('span'); span.textContent = d;
    ...
    card.addEventListener('click', () => { currentDate = new Date(year, month, 1); viewMode = 'month'; syncViewTabsUI(); renderCalendar(); });
    return card;
}
...
function getSelectedGregFor(prefix) {
    if (prefix === 'greg') {
        ...
        return { gy: gregState.gy, gm: gregState.gm, gd: gregState.selectedGd };
    }
    return null;
}
...
if (ev.icon) {
    ...
    const descIcon = document.getElementById('detail-desc-icon'); if (descIcon) {
...
function renderInlineChecklistItems(items) {
    ...
    items.forEach((item, index) => {
        const row = document.createElement('div'); row.className = 'checklist-inline-row';

        const cb = document.createElement('input'); cb.type = 'checkbox'; cb.className = 'neon-checkbox'; cb.checked = item.done;
        cb.addEventListener('change', function () { item.done = this.checked; });

        const textSpan = document.createElement('span'); textSpan.className = 'checklist-text'; textSpan.textContent = item.text;
        textSpan.setAttribute('contenteditable', 'true');
        textSpan.addEventListener('input', function () { item.text = this.textContent.trim(); });

        const delBtn = document.createElement('button'); delBtn.className = 'delete-item-btn'; delBtn.innerHTML = '✕';
        delBtn.addEventListener('click', () => { currentChecklistItems.splice(index, 1); renderInlineChecklistItems(currentChecklistItems); });

        const addSubBtn = document.createElement('button'); addSubBtn.className = 'add-subtask-btn'; addSubBtn.innerHTML = '+'; addSubBtn.title = 'Add subtask';
        addSubBtn.addEventListener('click', () => {
            ...
            if (subRows.length > 0) {
                const last = subRows[subRows.length - 1]; last.focus();
                const range = document.createRange(); range.selectNodeContents(last); range.collapse(false);
                const sel = window.getSelection(); sel.removeAllRanges(); sel.addRange(range);
            }
        });

        row.appendChild(cb); row.appendChild(textSpan); row.appendChild(addSubBtn); row.appendChild(delBtn);
        container.appendChild(row);

        if (item.subtasks && item.subtasks.length > 0) {
            item.subtasks.forEach((sub, subIndex) => {
                const subRow = document.createElement('div'); subRow.className = 'checklist-inline-row subtask-row';

                const subCb = document.createElement('input'); subCb.type = 'checkbox'; subCb.className = 'neon-checkbox'; subCb.checked = sub.done;
                subCb.addEventListener('change', function () { sub.done = this.checked; });

                const subText = document.createElement('span'); subText.className = 'checklist-text'; subText.textContent = sub.text;
                subText.setAttribute('contenteditable', 'true');
                subText.addEventListener('input', function () { sub.text = this.textContent.trim(); });

                const subDelBtn = document.createElement('button'); subDelBtn.className = 'delete-item-btn'; subDelBtn.innerHTML = '✕';
                subDelBtn.addEventListener('click', () => { item.subtasks.splice(subIndex, 1); renderInlineChecklistItems(currentChecklistItems); });

                subRow.appendChild(subCb); subRow.appendChild(subText); subRow.appendChild(subDelBtn);
...
function animateTabIndicator() {
    ...
    const tabRect = activeTab.getBoundingClientRect(), containerRect = viewTabsEl.getBoundingClientRect();
    gsap.to(tabIndicator, { left: tabRect.left - containerRect.left, width: tabRect.width, duration: 0.4, ease: 'power2.out' });
}
...
if (gregPrevBtn) gregPrevBtn.addEventListener('click', function(){
    gregState.gm--; if (gregState.gm < 0) { gregState.gm = 11; gregState.gy--; } renderGregPicker();
});
if (gregNextBtn) gregNextBtn.addEventListener('click', function(){
    gregState.gm++; if (gregState.gm > 11) { gregState.gm = 0; gregState.gy++; } renderGregPicker();
});
if (gregConfirmBtn) gregConfirmBtn.addEventListener('click', function(){
    if (!gregState.selectedGd) { gregDaysEl.style.outline = '1px solid var(--accent)'; return; }
    if (titlePickerActive) {
        titlePickerActive = false;
        var gy = gregState.gy, gm = gregState.gm, gd = gregState.selectedGd;
        var newDate = new Date(gy, gm, gd);
        if (viewMode === 'month') { newDate = new Date(newDate.getFullYear(), newDate.getMonth(), 1); }
        else if (viewMode === 'year')  { newDate = new Date(newDate.getFullYear(), 0, 1); }
        currentDate = newDate;
        gregPickerPopup.classList.remove('open');
        document.getElementById('greg-today-btn').style.display = 'none';
        renderCalendar();
...
if (gregPickerTrigger) gregPickerTrigger.addEventListener('click', function(){
    if (eventStartGreg.value) { var p = eventStartGreg.value.split('-'); openGregPicker(+p[0], +p[1] - 1, +p[2]); }
    else openGregPicker();
});
...
[eventModal].forEach(m => m?.addEventListener('click', e => { if (e.target === m) closeModal(m); }));
...
if (confirmYesBtn) confirmYesBtn.addEventListener('click', async () => {
    ...
    closeModal(eventDetailModal); hideDeleteConfirmation();
});
