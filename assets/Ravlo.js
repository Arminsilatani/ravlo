...
function isEventActiveOnDate(event, date) {
  ...
  if (matchesRule && event.recurrence_count) {
    const recType = event.recurrence_type.toLowerCase();
    let occurrenceNumber = 0;

    if (recType === "daily") {
      const diffDays = Math.floor(
        (date.getTime() - normalizedStart.getTime()) / (1000 * 60 * 60 * 24),
      );
      occurrenceNumber = Math.floor(diffDays / interval) + 1;
    } else if (recType === "weekly" || recType === "custom") {
      const activeDays = days.length > 0 ? days : [start.getDay()];
      if (activeDays.length === 0) {
        ...
        occurrenceNumber = Math.floor(diffWeeks / interval) + 1;
      } else {
        const startSunday = new Date(normalizedStart);
        startSunday.setDate(startSunday.getDate() - startSunday.getDay());
        const targetSunday = new Date(date);
        targetSunday.setDate(targetSunday.getDate() - targetSunday.getDay());
        const weekDiff = Math.floor(
          (targetSunday.getTime() - startSunday.getTime()) /
            (1000 * 60 * 60 * 24 * 7),
        );
        let count = 0;
        for (let w = 0; w <= weekDiff; w += interval) {
          ...
          for (let day of activeDays) {
            const occurrenceDate = new Date(sunday);
            occurrenceDate.setDate(occurrenceDate.getDate() + day);
      ...
      const diffMonths =
        (date.getFullYear() - start.getFullYear()) * 12 +
        (date.getMonth() - start.getMonth());
      occurrenceNumber = Math.floor(diffMonths / interval) + 1;
    } else if (recType === "yearly") {
      const diffYears = date.getFullYear() - start.getFullYear();
      occurrenceNumber = Math.floor(diffYears / interval) + 1;
    }

    if (occurrenceNumber > event.recurrence_count) {
      return false;
    }
  }
...
function renderInviteModalList() {
  ...
  if (!list) return;
  list.innerHTML = currentInvitees
...
function renderChecklistModalItems() {
  ...
  if (!container) return;
  container.innerHTML = tempChecklistItems
    .map(
      (item, i) => `
  ...
  events.forEach((ev) => {
    ...
    function buildTitle(event) {
      ...
      if (event.invitation_status === "pending" && event.parent_event_id) {
        return (
          base +
          ' <span style="font-size:10px;font-weight:400;background:#3A3A3A;color:#CCCCCC;padding:1px 4px;border-radius:3px;margin-left:4px;">Invited</span>'
        );
      }
      return base;
    }
...
function getNextSmartDate(ev) {
  ...
  const otherEvents = events.filter(
    (e) => e.id !== ev.id && e.recurrence_type !== "smart",
  );
...
async function createNextSmartTask(ev, newDate) {
  ...
  const payload = {
    ...
    end_date: ev.end_date
      ? new Date(
          newDate.getTime() + (new Date(ev.end_date) - new Date(ev.start_date)),
        ).toISOString()
      : null,
...
document
  ...
  ?.addEventListener("click", async function () {
    ...
    if (!hasMinRole(currentUserRole)) {
      ...
      showAccessDenied(
        "Access denied. You need at least Commander role to use Ravlo.",
      );
      return;
    }
...
function initLocationMap(containerId = "location-map") {
  ...
  locationMap.on("click", function (e) {
    ...
    locationMarker = icon
      ? L.marker(e.latlng, { icon }).addTo(locationMap)
      : L.marker(e.latlng).addTo(locationMap);
    const lat = e.latlng.lat.toFixed(6);
    const lng = e.latlng.lng.toFixed(6);
    const coordsInput =
      document.getElementById("location-modal-coords-input") ||
      document.getElementById("location-coords-input");
...
function updateMapFromCoords(lat, lng) {
  ...
  const icon = getAccentPinIcon();
  locationMarker = icon
    ? L.marker([lat, lng], { icon }).addTo(locationMap)
...
function openRecurrenceModal() {
  ...
  if (recurrence.type === "smart") {
    const radio = document.querySelector(
      `input[name="smartInterval"][value="${recurrence.smartInterval}"]`,
    );
  ...
  if (countInput) {
    countInput.value = recurrence.count || "";
  }
...
function updateRecurrenceModalUI() {
  ...
  if (countRow) countRow.style.display = showCount ? "flex" : "none";
  ...
  if (countInput) {
    countInput.value =
      recurrence.count !== null && recurrence.count !== undefined
        ? recurrence.count
        : "";
  }
}
...
function renderRecurrenceDays() {
  ...
  daysOfWeek.forEach((day) => {
    ...
    btn.addEventListener("click", () => {
      ...
      recurrence.days = Array.from(
        container.querySelectorAll(".rec-day-btn.selected"),
      ).map((b) => {
...
function renderYearPanel(
  ...
  container.innerHTML = "";
  var now = new Date();
  var thisYear = persian
    ? toJalali(now.getFullYear(), now.getMonth(), now.getDate()).jy
    : now.getFullYear();
...
function setDefaultTimeForToday(prefix, gy, gm, gd, force = false) {
  ...
  if (
    ...
    if (endHourEl)
      endHourEl.value = String((startHour + 1) % 24).padStart(2, "0");
...
function updateAllDayAndTimeRows() {
  ...
  const gregTimeRow = document.querySelector(
    "#greg-picker-popup .picker-time-row",
  );
...
function openGregPicker(gy, gm, gd) {
  ...
  if (gregState.selectedGd !== null) {
    setDefaultTimeForToday(
      "greg",
      gregState.gy,
      gregState.gm,
      gregState.selectedGd,
      false,
    );
  } else {
    setDefaultTimeForToday(
...
function renderGregorianMonth() {
  ...
  for (var i = 0; i < firstDay; i++)
    calendarGrid.appendChild(
      makeGregCell(year, month - 1, daysInPrev - firstDay + 1 + i, true),
    );
  for (var d = 1; d <= daysInMonth; d++) {
    var isToday =
      d === today.getDate() &&
  ...
  for (var t = 1; t <= trailing; t++)
    calendarGrid.appendChild(makeGregCell(year, month + 1, t, true));
}
...
function makeGregCell(year, month, day, otherMonth, isToday) {
  ...
  cell.className =
    "day-cell" + (otherMonth ? " other-month" : "") + (isToday ? " today" : "");
  ...
  dayEvents.forEach((ev) => {
    ...
    if (
      ev.recurrence_type &&
      ev.recurrence_type !== "none" &&
      ev.recurrence_type !== "smart"
    ) {
      var dateStr = toLocalDateString(new Date(ny, nm, nd));
      isCompleted =
        ev.completed_occurrences && ev.completed_occurrences.includes(dateStr);
    } else {
...
function renderDayView() {
  ...
  if (
    vy === todayCheck.getFullYear() &&
    vm === todayCheck.getMonth() &&
    vd === todayCheck.getDate()
  ) {
    if (currentUser) {
    ...
    if (overdueOccurrences.length > 0) {
      ...
      overdueOccurrences.forEach((item) => {
        ...
        titleSpan.textContent =
          (item.ev.title || "Untitled") + " (" + item.date + ")";
  ...
  requestAnimationFrame(function () {
    var eventsWithMinutes = timedEvents
      .map((ev) => {
        var start = new Date(ev.start_date);
        var end = ev.end_date
          ? new Date(ev.end_date)
    ...
    function getHeightBetween(startMin, endMin) {
      return (
        getCumulativeHeightUntil(endMin) - getCumulativeHeightUntil(startMin)
      );
    }
    ...
    components.forEach((comp) => {
      ...
      compItems.forEach((item) => {
        ...
        if (
          item.ev.location &&
          item.ev.location.lat != null &&
          item.ev.location.lng != null &&
          isLeafletReady()
        ) {
        ...
        if (
          item.ev.edit_request_status === "pending" &&
          !item.ev.parent_event_id
        ) {
        ...
        if (item.ev.icon) {
          ...
          titleSpan.appendChild(
            document.createTextNode(" " + (item.ev.title || "Event")),
          );
        } else {
        ...
        if (
          item.ev.parent_event_id &&
          item.ev.invitation_status === "accepted"
        ) {
          ...
          if (item.ev.type === "task" && item.ev.recurrence_type === "smart") {
            const isSmartDone =
              item.ev.status === "done" || item.ev.status === "completed";
            ...
            var isDone =
              item.ev.completed_occurrences &&
              Array.isArray(item.ev.completed_occurrences)
            ...
            endBtn.addEventListener("click", function (e) {
              ...
              if (isDone) {
                ...
                if (
                  item.ev.completed_timestamps &&
                  item.ev.completed_timestamps[occDate]
                ) {
                  delete item.ev.completed_timestamps[occDate];
                }
                if (
                  !item.ev.recurrence_type ||
                  item.ev.recurrence_type === "none"
                ) {
                  item.ev.status = "pending";
                  item.ev.completed_at = null;
                }
                ...
                updateEventInDB(item.ev.id, {
                  ...
                  completed_at: item.ev.completed_at,
                })
                  .then(() => {
                    if (currentUser)
                      removeNotificationsForEvent(item.ev.id, currentUser.id);
                  })
                  .catch(function () {});
              } else {
                if (!item.ev.completed_occurrences)
                  item.ev.completed_occurrences = [];
                item.ev.completed_occurrences.push(occDate);
                if (!item.ev.completed_timestamps)
                  item.ev.completed_timestamps = {};
                item.ev.completed_timestamps[occDate] =
                ...
                if (
                  ...
                  item.ev.status = "done";
                  item.ev.completed_at = new Date().toISOString();
                }
...
function renderYearView() {
  ...
  for (var m = 0; m < 12; m++)
    calendarGrid.appendChild(makeYearMonthCard_Greg(year, m, today));
}
...
function makeYearMonthCard_Greg(year, month, today) {
  ...
  for (var i = 0; i < firstDay; i++)
    miniGrid.appendChild(document.createElement("span"));
  for (var d = 1; d <= daysInMonth; d++) {
...
function setEventType(type) {
  ...
  const activeLabel = toggle.querySelector(
    `.event-type-label[data-type="${type}"]`,
  );
...
function validateTimeInput(prefix, isEnd = false) {
  const hourEl = document.getElementById(
    prefix + (isEnd ? "-end-hour" : "-hour"),
  );
  const minEl = document.getElementById(
    prefix + (isEnd ? "-end-minute" : "-minute"),
  );
  if (!hourEl || !minEl) return;
...
function resetEventForm() {
  ...
  currentChecklistItems = [];
  hideInlineChecklist();
  const checklistToggleBtn = document.getElementById(
    "toggle-checklist-mode-btn",
  );
...
function openNewEventAtTime(date) {
  ...
  gregTriggerText.textContent =
    GREG_MONTH_NAMES[month] +
    " " +
    day +
    ", " +
    year +
...
async function openEditModal(ev) {
  ...
  recurrence.count = ev.recurrence_count || null;
  updateRecurrencePreview();

  const countInput = document.getElementById("recurrence-count-input");
  if (countInput) countInput.value = recurrence.count || "";
  ...
  if (
    ev.invitee_ids &&
    Array.isArray(ev.invitee_ids) &&
    ev.invitee_ids.length > 0
  ) {
    try {
  ...
  if (ev.start_date) {
    ...
    if (ev.end_date) {
      ...
      document.getElementById("greg-end-minute").value = pad(
        endDate.getMinutes(),
      );
    } else {
    ...
    saveEventBtn.onclick = async function () {
      const newPayload = {
        ...
        start_date: eventStartInput?.value
          ? new Date(eventStartInput.value).toISOString()
          : ev.start_date,
        end_date: eventEndInput?.value
          ? new Date(eventEndInput.value).toISOString()
          : ev.end_date,
        all_day:
          document.getElementById("event-all-day-greg")?.checked || false,
...
async function saveEvent() {
  ...
  try {
    ...
    var payload = {
      ...
      recurrence_days:
        recurrence.type === "weekly" || recurrence.type === "custom"
          ? recurrence.days
          : [],
      recurrence_smart_interval:
        recurrence.type === "smart" ? recurrence.smartInterval : null,
      recurrence_count: recurrence.count,
      invitees:
        eventType === "event" ? currentInvitees.map((inv) => inv.name) : [],
      checklist: eventType === "task" ? currentChecklistItems : [],
    };

    if (
      eventType === "event" &&
      (currentLocationCoords || currentLocationName)
    ) {
      payload.location = {
    ...
    var eventId = null;
    var wasEditing = Boolean(editingEventId);

    if (editingEventId) {
      eventId = editingEventId;
      await updateEventInDB(eventId, payload);
      ...
      if (saved) {
        ...
        if (currentInvitees.length > 0) {
          for (const invitee of currentInvitees) {
            ...
            const { error: invErr } = await sb.rpc("insert_invited_event", {
              payload: invitePayload,
            });

            if (invErr)
              console.warn(
                "Failed to insert invitee row:",
...
async function approveEditRequest(eventId) {
  ...
  try {
    ...
    const allIds = [ev.user_id, ...(ev.invitee_ids || [])].filter(
      (id) => id !== ev.edit_request_by,
    );
    const editorName =
      [currentProfile?.first_name, currentProfile?.last_name]
        .filter(Boolean)
...
async function openEventDetail(ev, occurrenceDate) {
  ...
  const detailHeader = document.querySelector(
    "#event-detail-modal .detail-header",
  );
  ...
  let checklistContainer = document.getElementById(
    "detail-checklist-container",
  );
  if (!checklistContainer) {
    const descContainer2 = document.getElementById(
    ...
    if (descContainer2 && descContainer2.parentNode) {
      ...
      descContainer2.parentNode.insertBefore(
        checklistContainer,
        descContainer2.nextSibling,
      );
    }
  }
  if (checklistContainer) {
    ...
    if (ev.type === "event") {
      ...
      if (ev.parent_event_id) {
        ...
        if (error || !parentData) {
          ...
          attendeesList.innerHTML =
            '<div style="color:#FF6B6B;">Could not load attendees.</div>';
          return;
        }
      ...
      if (
        ...
        try {
          const { data: statuses } = await sb.rpc(
            "get_event_invitee_statuses",
            {
              p_event_id: ev.id,
            },
          );
      ...
      allPeople.forEach((person, i) => {
        const initials =
          person.name
            ...
            .join("")
            .toUpperCase()
            .slice(0, 2) || person.name[0].toUpperCase();
        ...
        row.innerHTML =
          avatarHtml +
          `<span class="attendee-name" style="margin-left:8px;">${person.name}${badge}</span>`;
  ...
  if (
    ...
    isCompleted =
      ev.completed_occurrences && Array.isArray(ev.completed_occurrences)
  ...
  if (cancelBtn) {
    if (isInvitee) {
      ...
      cancelBtn.onclick = () => {
        showConfirmModal(
          "Are you sure you want to delete this event? This will remove it for all guests.",
          () => {
            closeModal(eventDetailModal);
  ...
  if (completeBtn) {
    ...
    const isSmart = ev.type === "task" && ev.recurrence_type === "smart";
    const isRecurring =
      ev.recurrence_type && ev.recurrence_type !== "none" && !isSmart;

    if (isSmart) {
      const isSmartDone = ev.status === "done" || ev.status === "completed";
      completeBtn.textContent = isSmartDone ? "Undo" : "Done";
      completeBtn.onclick = async () => {
        if (isSmartDone) {
          ...
          renderCalendar();
          updateNotificationDot();
          showToast("Smart task undone.");
        } else {
          await completeSmartTask(ev);
          closeModal(eventDetailModal);
        }
      };
    } else {
      if (isCompleted) {
        ...
        completeBtn.onclick = async () => {
          if (dateForCompletion && isRecurring) {
            ...
            const occDate =
              dateForCompletion ||
              toLocalDateString(occurrenceDate || new Date());
            if (ev.completed_occurrences) {
              var idx = ev.completed_occurrences.indexOf(occDate);
              if (idx > -1) ev.completed_occurrences.splice(idx, 1);
            }
            if (ev.completed_timestamps && ev.completed_timestamps[occDate]) {
              delete ev.completed_timestamps[occDate];
            }
            ev.status = "pending";
            ev.completed_at = null;
            await updateEventInDB(ev.id, {
        ...
        completeBtn.onclick = async () => {
          if (dateForCompletion && isRecurring) {
            ...
            var occDate =
              ...
              toLocalDateString(occurrenceDate || new Date());
            if (!ev.completed_occurrences) ev.completed_occurrences = [];
            if (!ev.completed_occurrences.includes(occDate)) {
              ev.completed_occurrences.push(occDate);
            }
            if (!ev.completed_timestamps) ev.completed_timestamps = {};
            ev.completed_timestamps[occDate] = new Date().toISOString();
          ...
          renderCalendar();
          updateNotificationDot();
          showToast("Marked as done. Auto‑deleted in 28 days.");
        };
      }
    }
  }
  ...
  if (ev.edit_request_status === "pending" && !ev.parent_event_id) {
    ...
    editReqContainer.style.cssText =
      "background:rgba(255,200,0,0.1); border:1px solid rgba(255,200,0,0.3); border-radius:8px; padding:12px; margin:12px 0;";
...
async function leaveInvitedEvent(ev) {
  ...
  try {
    ...
    if (parentEvent) {
      ...
      const updatedInvitees = (parentEvent.invitees || []).filter(
        (name) => name !== myName,
      );
      const updatedInviteeIds = (parentEvent.invitee_ids || []).filter(
        (id) => id !== myId,
      );
...
async function deleteEventCascade(eventId) {
  ...
  try {
    ...
    events = events.filter(
      (e) => e.id !== eventId && e.parent_event_id !== eventId,
    );
...
function markTaskDone(ev) {
  ...
  const todayStr = toLocalDateString(new Date());
  if (!ev.completed_occurrences) ev.completed_occurrences = [];
  if (!ev.completed_occurrences.includes(todayStr)) {
    ev.completed_occurrences.push(todayStr);
  }
  if (!ev.completed_timestamps) ev.completed_timestamps = {};
  ev.completed_timestamps[todayStr] = new Date().toISOString();
...
document
  .getElementById("auth-continue-btn")
  ?.addEventListener("click", async function () {
    ...
    showGlobalLoader();
    try {
      const { data: exists, error: rpcError } = await sb.rpc(
        "check_email_exists",
...
document
  .getElementById("auth-send-reset-btn")
  ?.addEventListener("click", async function () {
...
document
  .getElementById("reg-to-login-btn")
...
document
  .getElementById("recurrence-select")
  ?.addEventListener("change", function () {
    ...
    if (
      (this.value === "weekly" || this.value === "custom") &&
      recurrence.days.length === 0
    ) {
...
document
  .getElementById("recurrence-count-input")
  ?.addEventListener("input", function () {
    const val = parseInt(this.value);
    recurrence.count = this.value.trim() === "" || isNaN(val) ? null : val;
  });
...
document
  .getElementById("recurrence-done-btn")
  ?.addEventListener("click", () => {
    const smartRadio = document.querySelector(
      'input[name="smartInterval"]:checked',
    );
...
document
  .getElementById("recurrence-modal-close")
  ?.addEventListener("click", () => {
    closeModal(document.getElementById("recurrence-modal"));
    updateRecurrencePreview();
  });
document
  .getElementById("recurrence-modal")
  ?.addEventListener("click", function (e) {
    if (e.target === this) {
      closeModal(this);
      updateRecurrencePreview();
...
document
  .getElementById("open-recurrence-btn")
  ?.addEventListener("click", openRecurrenceModal);
  ...
  ?.addEventListener("click", function () {
    ...
    if (btn.classList.contains("active")) {
      ...
      const activeColor = document.querySelector(
        `.tag-color-item[data-color="${selectedTagColor}"]`,
      );
...
if (gregPrevBtn)
  gregPrevBtn.addEventListener("click", function () {
    gregState.gm--;
    if (gregState.gm < 0) {
      gregState.gm = 11;
      gregState.gy--;
...
if (gregNextBtn)
  gregNextBtn.addEventListener("click", function () {
    gregState.gm++;
    if (gregState.gm > 11) {
      gregState.gm = 0;
      gregState.gy++;
...
if (gregConfirmBtn)
  gregConfirmBtn.addEventListener("click", function () {
    ...
    if (allDayChecked) {
      gregTriggerText.textContent =
        GREG_MONTH_NAMES[gregState.gm] + " " + gd + ", " + gy;
    } else {
      gregTriggerText.textContent =
        GREG_MONTH_NAMES[gregState.gm] +
        " " +
        gd +
        ", " +
        gy +
...
if (gregPickerPopup)
  gregPickerPopup.addEventListener("click", function (e) {
...
if (gregPickerTrigger)
  gregPickerTrigger.addEventListener("click", function () {
...
closeModalBtns.forEach((btn) =>
  btn.addEventListener("click", () => closeModal(eventModal)),
);
eventDetailModal.addEventListener("click", (e) => {
  if (e.target === eventDetailModal) closeModal(eventDetailModal);
});
[eventModal].forEach((m) =>
...
if (gregMonthBtn)
  gregMonthBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    openGregMonthPopup();
  });
if (gregYearBtn)
  gregYearBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    openGregYearPopup();
  });
...
if (confirmNoBtn)
  confirmNoBtn.addEventListener("click", hideDeleteConfirmation);
...
document.addEventListener("click", function (e) {
  ...
  if (g && !isEnd && isToday(g.gy, g.gm, g.gd)) {
    ...
    let testHour = targetId.includes("hour")
      ? newVal
      : parseInt(startHourEl?.value || 0);
    let testMin = targetId.includes("minute")
      ? newVal
      : parseInt(startMinuteEl?.value || 0);
...
document
  .getElementById("event-all-day-greg")
  ?.addEventListener("change", function () {
...
document
  .getElementById("toggle-icon-btn")
  ?.addEventListener("click", function () {
    const btn = this;
    if (btn.classList.contains("active")) {
      selectedIcon = null;
...
function rebuildInviteModal() {
  ...
  const closeBtn =
    content.querySelector("#invite-modal-close")?.outerHTML || "";
  content.innerHTML = `
        ${closeBtn}
  ...
  let searchTimeout;
  document
    .getElementById("invite-search-input")
    .addEventListener("input", function () {
...
function updateInviteSelectedUI() {
  ...
  container.innerHTML = currentInvitees
    .map(
      (inv) => `
...
document
  .getElementById("toggle-location-btn")
  ?.addEventListener("click", function () {
    const btn = this;
    if (btn.classList.contains("active")) {
      currentLocationCoords = null;
    ...
    locationSearchTimer = setTimeout(async () => {
      try {
        ...
        data.features.forEach((place) => {
          ...
          li.innerHTML =
            `<span style="font-weight:600; color:#FFF;">${mainName}</span>` +
            (shortAddress
              ? `<br><span style="font-size:11px; color:#888;">${shortAddress}</span>`
              : "");
...
document
  .getElementById("location-modal-close")
  ?.addEventListener("click", () => {
    closeModal(document.getElementById("location-modal"));
  });

document
  .getElementById("copy-invite-code-btn")
  ?.addEventListener("click", () => {
...
document
  .getElementById("location-coords-input")
  ?.addEventListener("input", function () {
...
function createChecklistRow(item, index, isSubtask) {
  ...
  textSpan.addEventListener("input", () => {
    item.text = textSpan.textContent.trim();
  });
  ...
  addSubBtn.addEventListener("click", () => {
    ...
    renderInlineChecklistItems(currentChecklistItems);
    setTimeout(() => {
      const lastSubText = document.querySelector(
        "#checklist-inline-items .subtask-row:last-of-type .checklist-text",
      );
...
function getLastOccurrenceDate(ev) {
  if (!ev.start_date || !ev.recurrence_count) return null;
  const start = new Date(ev.start_date);
  const count = ev.recurrence_count;
  const interval = ev.recurrence_interval || 1;

  if (ev.recurrence_type === "daily") {
    const last = new Date(start);
    last.setDate(last.getDate() + (count - 1) * interval);
    return last;
  } else if (ev.recurrence_type === "monthly") {
    const last = new Date(start);
    last.setMonth(last.getMonth() + (count - 1) * interval);
    return last;
  } else if (ev.recurrence_type === "yearly") {
    const last = new Date(start);
    last.setFullYear(last.getFullYear() + (count - 1) * interval);
    return last;
  } else if (
    ev.recurrence_type === "weekly" ||
    ev.recurrence_type === "custom"
  ) {
    const last = new Date(start);
    last.setDate(last.getDate() + (count - 1) * 7 * interval);
    return last;
  }
  return null;
}
...
async function cleanupOldCompletions() {
  ...
  for (let i = events.length - 1; i >= 0; i--) {
    ...
    try {
      if (ev.recurrence_type && ev.recurrence_type !== "none") {
        if (
          ev.completed_timestamps &&
          Object.keys(ev.completed_timestamps).length > 0
        ) {
          let changed = false;
          for (let dateStr in ev.completed_timestamps) {
            ...
            if (completedAt < twentyEightDaysAgo) {
              if (ev.completed_occurrences) {
                ev.completed_occurrences = ev.completed_occurrences.filter(
                  (d) => d !== dateStr,
                );
              }
        ...
        const lastOccurrence = getLastOccurrenceDate(ev);
        if (lastOccurrence && lastOccurrence < twentyEightDaysAgo) {
          const ok = await deleteEventFromDB(ev.id);
          if (ok) {
            events.splice(i, 1);
            console.log(
              `Deleted recurring event ${ev.id} (${ev.title}) after last occurrence.`,
            );
          }
        }
      ...
      const hasCompletionOccurrence =
        ev.completed_occurrences &&
        Array.isArray(ev.completed_occurrences) &&
        ev.completed_occurrences.length > 0;

      if (!hasExplicitStatus && !hasCompletionOccurrence) continue;

      let completedAt = null;

      if (ev.completed_at) {
        completedAt = new Date(ev.completed_at);
      } else if (
        ev.completed_timestamps &&
        Object.keys(ev.completed_timestamps).length > 0
      ) {
        const timestamps = Object.values(ev.completed_timestamps).map(
          (d) => new Date(d),
        );
        completedAt = new Date(Math.max(...timestamps));
      } else if (ev.start_date) {
        completedAt = new Date(ev.start_date);
      }

      if (!completedAt) {
        console.warn("No completion time found for event:", ev.id);
        continue;
      }

      if (completedAt < twentyEightDaysAgo) {
        console.log(
          `Deleting old completed event ${ev.id} (${ev.title}) - completed at ${completedAt}`,
        );
        const ok = await deleteEventFromDB(ev.id);
        if (ok) {
          events.splice(i, 1);
          ...
          console.error("Failed to delete from DB:", ev.id);
        }
      } else {
        if (!hasExplicitStatus && hasCompletionOccurrence) {
          await updateEventInDB(ev.id, {
            status: ev.type === "task" ? "done" : "completed",
            completed_at: completedAt.toISOString(),
          });
          ev.status = ev.type === "task" ? "done" : "completed";
          ev.completed_at = completedAt.toISOString();
        }
      }
    } catch (err) {
      console.error("Error in cleanup for event", ev.id, err);
    }
  }
  ...
  try {
    ...
    console.warn("Failed to delete declined invitations:", err);
  }
}
...
function openInvitationResponse(ev) {
  document.getElementById("invitation-response-title").textContent =
    "Event Invitation";
  ...
  if (inviterId) {
    ...
    document.getElementById("invitation-response-message").textContent =
...
function applyPostponeOffset(minutes) {
  ...
  const newStart = new Date(
    new Date(ev.start_date).getTime() + minutes * 60000,
  );
...
document
  .getElementById("postpone-custom-apply")
  .addEventListener("click", function () {
    const mins = parseInt(
      document.getElementById("postpone-custom-input").value,
    );
...
document
  .getElementById("postpone-modal-close")
  .addEventListener("click", () => {
    closeModal(document.getElementById("postpone-modal"));
  });
